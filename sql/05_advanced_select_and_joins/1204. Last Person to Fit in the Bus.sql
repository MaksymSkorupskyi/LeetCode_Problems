...
-- +------+----+-----------+--------+--------------+
WITH
  cte_running_total_weight AS (
    SELECT
      person_name,
      SUM(weight) OVER(ORDER BY turn ROWS UNBOUNDED PRECEDING)  AS running_total_weight
    FROM
      Queue
  )
...
SELECT
  person_name
FROM
  cte_running_total_weight
WHERE
  running_total_weight <= 1000
ORDER BY
  running_total_weight DESC
LIMIT
  1
